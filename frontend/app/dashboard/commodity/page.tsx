    ...
    >
        ...
        <div className="relative bg-gradient-to-r from-[var(--shell-panel)] via-[var(--shell-card)] to-[var(--shell-card-2)] border border-[var(--shell-border)] rounded-2xl p-6 overflow-hidden">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[var(--shell-border-2)] mx-1">|</span>
              {RANGES.map((r) => (
                <button
                  key={r.value}
      ...
      <motion.div variants={fadeUp}>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Latest Price */}
          <div className="bg-gradient-to-br from-[var(--shell-card)] to-[var(--shell-border)] border border-[#38BDF8]/30 rounded-xl p-5 transition-all hover:shadow-lg hover:shadow-[#38BDF8]/5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[0.6rem] font-bold tracking-[0.1em] text-[var(--shell-muted-2)] uppercase">Latest Price</span>
              <DollarSign className="w-5 h-5 text-[#38BDF8]" />
            </div>
            {loading ? (
              <div className="h-7 w-20 bg-[var(--shell-border-2)] rounded animate-pulse" />
            ) : (
              <>
          ...
          {/* Market Direction */}
          <div className={`bg-gradient-to-br from-[var(--shell-card)] to-[var(--shell-border)] border rounded-xl p-5 transition-all hover:shadow-lg ${
            ...
            <div className="flex items-center justify-between mb-3">
              <span className="text-[0.6rem] font-bold tracking-[0.1em] text-[var(--shell-muted-2)] uppercase">Market Signal</span>
            ...
            {loading ? (
              <div className="h-7 w-16 bg-[var(--shell-border-2)] rounded animate-pulse" />
            ) : (
              <>
                <div className={`text-2xl font-extrabold ${pred === "UP" ? "text-[#22C55E]" : pred === "DOWN" ? "text-[#EF4444]" : "text-[#F59E0B]"}`}>{pred}</div>
                <p className="text-xs text-[var(--shell-muted-2)] mt-1">{kpi?.model_prediction ? "AI Model" : "Headline-based"}</p>
              </>
            )}
          </div>

          {/* Confidence */}
          <div className="bg-gradient-to-br from-[var(--shell-card)] to-[var(--shell-border)] border border-[#A78BFA]/30 rounded-xl p-5 transition-all hover:shadow-lg hover:shadow-[#A78BFA]/5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[0.6rem] font-bold tracking-[0.1em] text-[var(--shell-muted-2)] uppercase">Confidence</span>
            ...
            {loading ? (
              ...
              <>
                ...
                <p className="text-xs text-[var(--shell-muted-2)] mt-1">{confPct >= 75 ? "High confidence" : confPct >= 55 ? "Moderate" : "Low confidence"}</p>
              </>
            )}
          </div>

          {/* Avg Sentiment */}
          <div className="bg-gradient-to-br from-[var(--shell-card)] to-[var(--shell-border)] border border-[#F59E0B]/30 rounded-xl p-5 transition-all hover:shadow-lg hover:shadow-[#F59E0B]/5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[0.6rem] font-bold tracking-[0.1em] text-[var(--shell-muted-2)] uppercase">Avg Sentiment</span>
            ...
            {loading ? (
              ...
              <>
                <div className={`text-2xl font-extrabold ${sentiment > 0.1 ? "text-[#22C55E]" : sentiment < -0.1 ? "text-[#EF4444]" : "text-[var(--shell-muted)]"}`}>
                  {sentiment > 0 ? "+" : ""}{sentiment.toFixed(3)}
                </div>
                <p className="text-xs text-[var(--shell-muted-2)] mt-1">{kpi?.total_headlines_24h ?? 0} headlines analyzed</p>
              </>
            )}
          </div>
        </div>
            ...
            {loading || !priceStats ? (
              <div className="space-y-3 animate-pulse">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="h-4 bg-[var(--shell-border-2)] rounded w-3/4" />
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[var(--shell-bg)]/50 rounded-lg p-3">
                  <p className="text-[0.6rem] text-[var(--shell-muted-3)] uppercase tracking-wide mb-1">Period High</p>
                  <p className="text-lg font-bold text-[#22C55E]">${priceStats.high.toFixed(2)}</p>
                </div>
                <div className="bg-[var(--shell-bg)]/50 rounded-lg p-3">
                  <p className="text-[0.6rem] text-[var(--shell-muted-3)] uppercase tracking-wide mb-1">Period Low</p>
                  <p className="text-lg font-bold text-[#EF4444]">${priceStats.low.toFixed(2)}</p>
                </div>
                <div className="bg-[var(--shell-bg)]/50 rounded-lg p-3">
                  <p className="text-[0.6rem] text-[var(--shell-muted-3)] uppercase tracking-wide mb-1">Average</p>
                  <p className="text-lg font-bold text-[var(--shell-muted)]">${priceStats.avg.toFixed(2)}</p>
                </div>
                <div className="bg-[var(--shell-bg)]/50 rounded-lg p-3">
                  <p className="text-[0.6rem] text-[var(--shell-muted-3)] uppercase tracking-wide mb-1">Spread (H-L)</p>
            ...
            {loading ? (
              <div className="space-y-3 animate-pulse">
                {Array.from({ length: 3 }).map((_, i) => (
                  <div key={i} className="h-6 bg-[var(--shell-border-2)] rounded" />
                ))}
              </div>
            ) : (
              <div className="space-y-3">
                ...
                <div>
                  <div className="flex items-center justify-between text-xs mb-1">
                    ...
                    <span className="text-[var(--shell-muted)]">{sentimentBreakdown.positive} ({sentimentBreakdown.positivePct}%)</span>
                  </div>
                  <div className="h-2.5 bg-[var(--shell-bg)] rounded-full overflow-hidden">
                ...
                <div>
                  <div className="flex items-center justify-between text-xs mb-1">
                    ...
                    <span className="text-[var(--shell-muted)]">{sentimentBreakdown.neutral} ({sentimentBreakdown.neutralPct}%)</span>
                  </div>
                  <div className="h-2.5 bg-[var(--shell-bg)] rounded-full overflow-hidden">
                ...
                <div>
                  <div className="flex items-center justify-between text-xs mb-1">
                    ...
                    <span className="text-[var(--shell-muted)]">{sentimentBreakdown.negative} ({sentimentBreakdown.negativePct}%)</span>
                  </div>
                  <div className="h-2.5 bg-[var(--shell-bg)] rounded-full overflow-hidden">
            ...
            {loading ? (
              <div className="space-y-2 animate-pulse">
                {Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className="h-16 bg-[var(--shell-bg)]/50 rounded-lg" />
                ))}
              </div>
            ) : headlines.length === 0 ? (
              <p className="text-sm text-[var(--shell-muted-2)] text-center py-8">No headlines available for {commodity}.</p>
            ) : (
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                            <span className="text-[0.6rem] text-[var(--shell-muted-3)]">{h.source}</span>
                            <span className="text-[0.6rem] text-[var(--shell-border-2)]">•</span>
                            <span className="text-[0.6rem] text-[var(--shell-muted-3)]">{h.event_type}</span>
                            <span className="text-[0.6rem] text-[var(--shell-border-2)]">•</span>
